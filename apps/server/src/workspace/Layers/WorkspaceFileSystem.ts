...
function isLexicallyOutsideRoot(relativePath: string, path: Path.Path): boolean {
  const normalizedInputPath = relativePath.trim();
  if (path.isAbsolute(normalizedInputPath)) {
    return true;
  }
  const normalizedRelativePath = normalizedInputPath
    ? normalizedInputPath.replaceAll("\\", "/")
    : ".";
  const normalizedPath = path.normalize(normalizedRelativePath).replaceAll("\\", "/");
      ...
      const target = yield* workspacePaths
        ...
        .pipe(
          Effect.mapError((cause) => {
            if (isLexicallyOutsideRoot(input.relativePath, path)) {
              return cause;
            }
