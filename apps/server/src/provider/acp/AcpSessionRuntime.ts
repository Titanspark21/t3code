    ...
    return {
      ...
      prompt: (payload) =>
        Effect.gen(function* () {
          ...
          const promptRpcFiber = yield* promptStartCancelSemaphore.withPermit(
            Effect.gen(function* () {
      ...
      cancel: getStartedState.pipe(
        Effect.flatMap((started) =>
          promptStartCancelSemaphore.withPermit(
            Effect.gen(function* () {
              const activePromptFibers = yield* Ref.get(activePromptFibersRef);
              yield* Effect.forEach(activePromptFibers, (fiber) =>
                Fiber.interrupt(fiber).pipe(Effect.ignore),
              );
              const payload = { sessionId: started.sessionId };
              yield* runLoggedRequest("session/cancel", payload, acp.agent.cancel(payload)).pipe(
                Effect.ignore,
              );
            }),
          ),
