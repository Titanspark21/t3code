...
import { type CodexRateLimitSnapshot, codexRateLimitsToUpdate } from "./codexUsageLimits.ts";
...
export interface CodexAdapterLiveOptions {
  ...
  readonly refreshQuota?: () => Effect.Effect<CodexRateLimitSnapshot | undefined, never>;
  ...
  const refreshQuota: NonNullable<CodexAdapterShape["refreshQuota"]> | undefined =
    options?.refreshQuota
      ? Effect.fn("refreshCodexQuota")(function* () {
          ...
          // Normalize before the event leaves the adapter, exactly as the
          // streamed rate-limit path does.
          const limits = codexRateLimitsToUpdate(rateLimits);
