/**
 * AntigravityAdapterLive — Antigravity CLI (`agent acp`) via ACP.
 *
...
export interface AntigravityAdapterLiveOptions {
   ...
   * the `antigravitySettings` captured at construction.
   *
...
export function makeAntigravityAdapter(
  ...
  return Effect.gen(function* () {
    ...
    const mapAcpClientFailure = <A, E, R>(effect: Effect.Effect<A, E, R>) =>
      effect.pipe(
        Effect.mapError(
          (cause) =>
            new EffectAcpErrors.AcpTransportError({
              detail: "Failed to process Antigravity ACP client event.",
              cause,
            }),
        ),
      );
    ...
    const logNative = (
      ...
      method: string,
      payload: unknown,
      _source: "acp.jsonrpc" | "acp.antigravity.extension",
    ) =>
      Effect.gen(function* () {
    ...
    const startSession: AntigravityAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          // Resolve the AntigravitySettings used to spawn the ACP child. Production
          ...
          // `updateSettings({ providers: { antigravity: { binaryPath } } })` calls
          // actually take effect when the next session spawns.
          const effectiveAntigravitySettings = options?.resolveSettings
            ? yield* options.resolveSettings
            : antigravitySettings;

          const mcpSession = McpProviderSession.readMcpProviderSession(input.threadId);
          const acp = yield* makeAntigravityAcpRuntime({
            settings: effectiveAntigravitySettings,
          ...
          const started = yield* Effect.gen(function* () {
            yield* acp.handleUnknownExtNotification((method, params) =>
              mapAcpClientFailure(
                Effect.gen(function* () {
                  if (!isAntigravityRateLimitsMethod(method)) return;
                  yield* logNative(input.threadId, method, params, "acp.antigravity.extension");
                  yield* offerRuntimeEvent({
                    ...
                    threadId: input.threadId,
                    turnId: ctx?.activeTurnId,
                    payload: { rateLimits: params },
                    raw: {
                      source: "acp.antigravity.extension",
                      method,
                      payload: params,
                    },
                  });
              ...
              mapAcpClientFailure(
                Effect.gen(function* () {
                  ...
                  if (
                    ...
                    !isAntigravityProjectTrustRequest(params)
                  ) {
          ...
          yield* applyRequestedSessionConfiguration({
            runtime: acp,
            currentModelId: currentAntigravityModelIdFromSessionSetup(started.sessionSetupResult),
            runtimeMode: input.runtimeMode,
    ...
    const respondToUserInput: AntigravityAdapterShape["respondToUserInput"] = (_threadId) =>
      Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method: "session/elicitation",
          detail: "Antigravity ACP does not expose structured user-input requests.",
        }),
      );
