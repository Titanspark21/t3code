...
it.effect("KiloAdapter delegates startSession and stamps binaryPath", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const manager = new FakeKiloManager();
      const adapter = yield* makeKiloAdapter(enabledConfig, { manager });
...
it.effect("KiloAdapter rejects attachments", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const manager = new FakeKiloManager();
      const adapter = yield* makeKiloAdapter(enabledConfig, { manager });
...
it.effect("KiloAdapter forwards events through the stream", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const manager = new FakeKiloManager();
      const adapter = yield* makeKiloAdapter(enabledConfig, { manager });
...
it.effect("KiloAdapter refuses startSession when disabled", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const manager = new FakeKiloManager();
      const adapter = yield* makeKiloAdapter({ ...enabledConfig, enabled: false }, { manager });
      const result = yield* adapter
