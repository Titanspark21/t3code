    ...
    describe("checkClaudeProviderStatus", () => {
      ...
      it.effect("deduplicates probed claude slash commands by name", () =>
        Effect.gen(function* () {
          ...
          // This fork enriches known commands with syntax and support notes, so
          // compare on the dedupe behaviour under test rather than on the whole
          // enriched record.
          assert.deepStrictEqual(
            status.slashCommands.map((command) => command.name),
            [COMPACT_SLASH_COMMAND.name, "ui"],
          );
          assert.deepStrictEqual(status.slashCommands[1], {
            name: "ui",
            description: "Explore and refine UI",
            input: { hint: "component-or-screen" },
          });
