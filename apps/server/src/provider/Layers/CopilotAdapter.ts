...
// Inline capabilities — `getProviderCapabilities` was a fork-side helper
// that died with the upstream merge; the values it returned for "copilot"
// are reproduced inline at the adapter shape construction below.
...
// Local re-declaration of the codex reasoning-effort literal — the contracts
// package no longer exports `CodexReasoningEffort` after the upstream merge,
// so we mirror the literal set here. Strictly internal; never crosses a wire
// or storage boundary.
// TODO(sync): align with upstream's new model-options envelope once the
// fork's reasoning-effort UX is rewired against `ModelSelection.options`.
  ...
  Effect.gen(function* () {
    ...
    const startSession: CopilotAdapterShape["startSession"] = (input) =>
      Effect.gen(function* () {
        ...
        // Per-instance config captured in the driver factory closure.
        // Replaces the legacy `ServerSettingsService.getSettings` lookup
        // so two driver instances can run side-by-side with independent
        // binary paths and config dirs.
        const copilotSettings = config;
...
/**
 * Per-instance Copilot adapter factory used by `CopilotDriver.create()`.
 *
 * Returns the `CopilotAdapterShape` value directly — no Layer wrapper —
 * so the driver can compose it with snapshot/textGeneration closures and
 * hand the result back to the registry as one `ProviderInstance`.
 *
 * TODO(sync): the indirection through `makeCopilotAdapter` (no `Live`
 * suffix) and the legacy `CopilotAdapter` Service tag is a transitional
 * shim. A future cleanup should inline the body straight into
 * `CopilotDriver.ts` and delete the Service tag entirely. Tracked in the
 * sync-multiprovider PR description.
 */
export const makeCopilotAdapterImpl = (
  config: GenericProviderSettings,
  options?: CopilotAdapterLiveOptions,
) => makeCopilotAdapter(config, options);
