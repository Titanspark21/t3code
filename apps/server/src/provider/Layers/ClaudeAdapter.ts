...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const emitAccountUsageSnapshot = Effect.fn("emitAccountUsageSnapshot")(function* (
    ...
    yield* offerRuntimeEvent({
      ...
      // Upstream normalizes this event before it leaves the adapter, so the
      // quota consumer never sees Claude's wire shape.
      payload: {
        limits: {
          windows: claudeUsageResponseToLimits({ response: usage, checkedAt: stamp.createdAt })
            .limits.windows,
        },
      },
    });
