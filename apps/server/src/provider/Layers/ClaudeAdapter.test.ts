  ...
  it.effect("pulls all Claude quota windows on init and throttles binding-window refreshes", () => {
    ...
    return Effect.gen(function* () {
      ...
      if (event?.type === "account.rate-limits.updated") {
        // The adapter normalizes before emitting, so the event carries shared
        // usage windows rather than Claude's wire shape.
        assert.deepEqual(
          event.payload.limits.windows.map((window) => ({
            id: window.id,
            usedPercent: window.usedPercent,
            resetsAt: window.resetsAt,
          })),
          [
            { id: "five_hour", usedPercent: 22, resetsAt: "2026-08-24T05:00:00.000Z" },
            { id: "seven_day", usedPercent: 48, resetsAt: "2026-08-30T00:00:00.000Z" },
          ],
        );
      }
