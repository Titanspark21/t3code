  ...
  effectIt.effect.each([
    ...
    ([status, state]) =>
      Effect.gen(function* () {
        ...
        const rateLimited = yield* projectEvent(
          afterRunning,
          makeEvent({
            sequence: 4,
            type: "thread.session-set",
        ...
        expect(rateLimited.threads[0]?.session?.status).toBe("rate-limited");
        expect(rateLimited.threads[0]?.latestTurn?.state).toBe("error");
      }),
  );
