...
export const makeAntigravityTextGeneration = Effect.fn("makeAntigravityTextGeneration")(function* (
  ...
  const runAntigravityJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      const resolvedModel = resolveAntigravityAcpModelId(modelSelection.model);
        ...
        yield* applyAntigravityAcpModelSelection({
          ...
          mapError: (cause) =>
            new TextGenerationError({
              operation,
              detail: "Failed to set Antigravity ACP base model for text generation.",
              cause,
            }),
        });
          ...
          Option.match({
            onNone: () =>
              Effect.fail(
                new TextGenerationError({
                  operation,
                  detail: "Antigravity ACP request timed out.",
                }),
              ),
