...
export const getLogDir = DesktopIpc.makeIpcMethod({
  channel: IpcChannels.LOG_DIR_CHANNEL,
  payload: Schema.Void,
  result: Schema.String,
  handler: Effect.fn("desktop.ipc.window.getLogDir")(function* () {
    const environment = yield* DesktopEnvironment.DesktopEnvironment;
    return environment.logDir;
  }),
});

export const listLogFiles = DesktopIpc.makeIpcMethod({
  channel: IpcChannels.LOG_LIST_CHANNEL,
  payload: Schema.Void,
  result: Schema.Array(Schema.String),
  handler: Effect.fn("desktop.ipc.window.listLogFiles")(function* () {
    const environment = yield* DesktopEnvironment.DesktopEnvironment;
    const fileSystem = yield* FileSystem.FileSystem;
    return yield* fileSystem.readDirectory(environment.logDir).pipe(
      Effect.map((entries) =>
        entries.filter((entry) => entry.endsWith(".log") || entry.endsWith(".ndjson")).toSorted(),
      ),
...
export const readLogFile = DesktopIpc.makeIpcMethod({
  channel: IpcChannels.LOG_READ_CHANNEL,
  payload: Schema.String,
  result: Schema.String,
  handler: Effect.fn("desktop.ipc.window.readLogFile")(function* (filename) {
    const environment = yield* DesktopEnvironment.DesktopEnvironment;
    const basename = environment.path.basename(filename);
    if (basename !== filename || (!basename.endsWith(".log") && !basename.endsWith(".ndjson"))) {
      return "";
    }

    const fileSystem = yield* FileSystem.FileSystem;
    return yield* fileSystem
      .readFileString(environment.path.join(environment.logDir, basename))
      .pipe(Effect.orElseSucceed(() => ""));
  }),
});

export const openLogDir = DesktopIpc.makeIpcMethod({
  channel: IpcChannels.LOG_OPEN_DIR_CHANNEL,
  payload: Schema.Void,
  result: Schema.Void,
  handler: Effect.fn("desktop.ipc.window.openLogDir")(function* () {
    const environment = yield* DesktopEnvironment.DesktopEnvironment;
    const fileSystem = yield* FileSystem.FileSystem;
    const shell = yield* ElectronShell.ElectronShell;
