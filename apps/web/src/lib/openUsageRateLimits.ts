...
function toProviderDriverKind(providerId: string | undefined): ProviderDriverKind | null {
  if (providerId === "codex") return ProviderDriverKind.make("codex");
  if (providerId === "claude") return ProviderDriverKind.make("claudeAgent");
  if (providerId === "copilot") return ProviderDriverKind.make("copilot");
  if (providerId === "cursor") return ProviderDriverKind.make("cursor");
  if (providerId === "opencode") return ProviderDriverKind.make("opencode");
  if (providerId === "gemini") return ProviderDriverKind.make("geminiCli");
  if (providerId === "amp") return ProviderDriverKind.make("amp");
  if (providerId === "kilo") return ProviderDriverKind.make("kilo");
  return null;
}
...
export function normalizeOpenUsageSnapshot(
  snapshot: unknown,
  preferredProvider?: ProviderDriverKind | null,
): ProviderRateLimit | null {
  const parsed = asRecord(snapshot) as OpenUsageSnapshot | null;
  if (!parsed) return null;

  const provider =
    toProviderDriverKind(asString(parsed.providerId)) ??
