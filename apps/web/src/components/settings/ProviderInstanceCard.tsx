...
export function ProviderInstanceCard({
  ...
  // The fork shows each provider's remaining quota on hover; upstream's card
  // has no such slot, so the badge wraps the section icon.
  const titleIconWithQuotaNode =
    driverKind && quotaNowMs !== undefined ? (
      <ProviderQuotaTooltip
        driverKind={driverKind}
        nowMs={quotaNowMs}
        snapshot={quotaSnapshot}
        trigger={<div className="flex shrink-0">{titleIconNode}</div>}
      />
    ) : (
      titleIconNode
    );
