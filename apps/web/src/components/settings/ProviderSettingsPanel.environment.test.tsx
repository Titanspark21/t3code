...
// `useNowMinute` is backed by useSyncExternalStore, which this harness does
// not stub; the badge only needs a stable minute.
vi.mock("../../hooks/useNowMinute", () => ({
  useNowMinute: () => "2026-09-04T09:00",
}));

// The panel reads quota for the per-provider badge; this harness calls the
// component as a plain function, so the atom-backed hook is stubbed out.
vi.mock("../../state/quota", () => ({
  useQuota: () => ({
