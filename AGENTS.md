...
- [ ] **Continue the prior T3CODE-081 request: add durable scheduled tasks that let users choose model, prompt, time, and both C** — handed back on 2026-08-30 by t3code-7. <!-- job:6344cf12 -->
      ...
      Resume round 2 preserved and reviewed. Current branch/workspace is t3code#agent/6344cf12-continue-the-prior-t3code-081-requ with the scheduler implementation still uncommitted (plus a one-line fix to a pre-existing duplicate const in packages/contracts/src/orchestration.test.ts). Work verified before machine failure: quick bridge verify/typecheck PASSED across apps/packages/infra; focused tests PASSED: apps/server/src/scheduledTasks/Schedule.test.ts 10/10, packages/client-runtime/src/state/shellReducer.test.ts 10/10, packages/contracts/src/orchestration.test.ts 52/52, apps/server/src/orchestration/Layers/ProjectionSnapshotQuery.test.ts 21/21; git diff --check passed. Reviewed implementation: SQLite migration/history, server-scoped scheduler startup/recovery, timezone/DST/missed-run handling, retries, provider-instance model selection (supports separate Codex accounts), scheduler thread prefix filtering from shell/archive/search, automatic thread settlement and run history. I found two lifecycle issues that still need fixing before commit/landing: (1) runDueTasks waits for each full run (concurrency:1), so one long task can delay later scheduled tasks; launch each due run in a scoped background fiber while retaining activeRuns guard. Restart recovery similarly serializes running rows; recover rows concurrently/scoped so polling is not blocked behind a long resumed run. (2) restart recovery can wait up to six hours if crash occurred after thread.create but before thread.turn.start: waitForThread sees an existing thread with latestTurn null and no busy session; treat that restart-only state as interrupted/retryable (or otherwise resume turn deterministically). Also consider whether user-facing full snapshot needs scheduler filtering; command read model must keep scheduler threads for dispatch. A new unstaged ProjectionSnapshotQuery.test.ts change is present and passed; it verifies scheduler threads are excluded from active/archive navigation/search. DO NOT redo committed/preserved scheduler work. Machine blocker: Q.run now reports /usr/bin/git cannot be spawned, ENOENT, and explicitly says git is broken on the machine and to stop rather than work around it. Because git is unavailable, I could not apply the final lifecycle edits, commit_and_push, run full verify, finish_job, or claim_next_task. Resume from this exact preserved workspace after host git is repaired.
