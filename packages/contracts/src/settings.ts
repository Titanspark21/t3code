...
export const AmpSettings = makeProviderSettingsSchema(
  {
    ...
    binaryPath: makeBinaryPathSetting("amp").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description: "Path to the Amp CLI binary used by this instance.",
...
export const CopilotSettings = makeProviderSettingsSchema(
  {
    ...
    configDir: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Config directory",
        description:
          "Custom config directory used when running this Copilot instance. Keeps account state separate.",
        providerSettingsForm: { placeholder: "~/.copilot", clearWhenEmpty: "omit" },
      }),
    ),
    customModels: Schema.Array(Schema.String).pipe(
...
export const GeminiCliSettings = makeProviderSettingsSchema(
  {
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(true)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
